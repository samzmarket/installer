...
ubuntu_dep() {
  # Install deps untuk menambahkan repo
  install_packages "software-properties-common apt-transport-https ca-certificates gnupg"

  # Tambahkan repo Universe untuk Ubuntu
  add-apt-repository universe -y

  # Tambahkan PPA untuk PHP (gunakan 8.2)
  LC_ALL=C.UTF-8 add-apt-repository -y ppa:ondrej/php
}

debian_dep() {
  # Install dependensi untuk menambahkan repo
  install_packages "dirmngr ca-certificates apt-transport-https lsb-release"

  # Install PHP 8.2 dari repo Sury
...
# Perbarui fungsi `dep_install` untuk mengganti referensi PHP 8.1 menjadi PHP 8.2
dep_install() {
  output "Installing dependencies for $OS $OS_VER..."

  # Update repos sebelum instalasi
  update_repos
  ...
  ubuntu | debian)
    ...
    # Install dependensi
    install_packages "php8.2 php8.2-{cli,common,gd,mysql,mbstring,bcmath,xml,fpm,curl,zip} \
  ...
  rocky | almalinux)
    ...
    # Install dependensi
    install_packages "php8.2 php8.2-{common,fpm,cli,json,mysqlnd,mcrypt,gd,mbstring,pdo,zip,bcmath,dom,opcache,posix} \
    ...
    # Buat konfigurasi untuk php fpm
    php_fpm_conf
    ;;
  esac
...
# Ubah konfigurasi Nginx untuk menggunakan PHP 8.2
configure_nginx() {
